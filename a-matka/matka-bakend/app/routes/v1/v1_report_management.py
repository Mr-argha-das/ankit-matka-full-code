...
from app.models import Bid, DepositQR, Market, RateChart, Result, Transaction, User, Wallet, Withdrawal
...
@router.post("/bids/revert/{bid_id}", dependencies=[Depends(require_admin)])
def revert_bid(bid_id: str):
    bid = Bid.objects(id=bid_id).first()
    if not bid:
        raise HTTPException(404, "Bid not found")
    ...
    Transaction(
        ...
        user_id=bid.user_id,
        amount=bid.points,
        payment_method="Bid Revert Refund",
        status="SUCCESS",
        raw_response=f"Reverted bid {bid_id}",
    ).save()

    refunded_points = bid.points
    bid.delete()
