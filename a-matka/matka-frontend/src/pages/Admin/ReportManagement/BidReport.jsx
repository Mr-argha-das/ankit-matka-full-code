...
export default function BidReport() {
  ...
  const handleRevert = async (bid_id) => {
    if (!confirm("Revert this bid and refund points to user wallet?")) return;
    try {
      const res = await axios.post(
        `${API_URL}/admin/bids/revert/${bid_id}`,
        {},
        { headers }
      );
      alert(res.data?.message || "Bid reverted successfully");
      fetchBids();
    } catch (err) {
      console.error(err);
      alert(err.response?.data?.detail || "Failed to revert bid");
    }
  };
                    ...
                    >
                      ...
                      <td className="py-1.5 px-2">
                        <div className="flex flex-wrap gap-2">
                          <button
                            onClick={() => handleRevert(b.bid_id)}
                            className="bg-emerald-600 hover:bg-emerald-500 px-3 py-1 rounded text-white flex items-center gap-2"
                          >
